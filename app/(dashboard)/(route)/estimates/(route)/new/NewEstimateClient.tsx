...
const NewEstimateClient = () => {
  const { data: company, mutate: companyMutate } = useCompany();
  const companyData: Company = company;
  const { data: fetchedEstimates, mutate: estimatesMutate } = useEstimates();
  const [estimateData, setEstimateData] = useState(emptyEstimateField);
  ...
  useEffect(() => {
    setEstimateData((prevState: any) => ({
      ...prevState,
      estimateNumber: companyData?.currentEstimateNumber + "",
      date: date,
    }));
  }, [date, companyData]);
  ...
  const handleSubmit = useCallback(async () => {
    try {
      const { invoiceType: it, ...rest } = estimateData;
      ...
      estimatesMutate();
      companyMutate();
      const { invoiceType } = estimateData;
      setEstimateData(emptyEstimateField);
      setEstimateData((prevState: any) => ({
        ...prevState,
  ...
  }, [estimateData, items, invoiceCustomer, estimatesMutate, companyMutate]);
  ...
  return (
    <div className="container mx-auto xl:px-30 max-w-6xl border-2 p-3">
      <div className="flex justify-between">
        ...
        <div className="text-center text-md sm:text-xl">NEW ESTIMATE</div>
        <Button
          className="h-8"
          variant="default"
        ...
        <div>
          <Card className="w-[300px] md:w-[400px] p-4">
            <p className="mb-1 flex gap-2">
              <LiaFileInvoiceSolid size={20} /> Estimate Details
            </p>
            <Separator className="my-2" />
            <CardContent className="grid grid-cols-3 gap-1 p-0">
              <p className="pt-2">Estimate No.</p>
              <Input
        ...
        <div className="mt-7 flex justify-end">
          <Card className="w-[300px] md:w-[400px] p-4">
            ...
            <CardContent className="grid grid-cols-2 gap-1 p-0">
              ...
              {estimateData.invoiceType === "state" ? (
                <>
...
export default NewEstimateClient;
